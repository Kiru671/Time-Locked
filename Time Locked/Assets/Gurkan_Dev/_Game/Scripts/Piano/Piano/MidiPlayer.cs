	...
	void Update ()
	{
		...
		if (_midi != null && MidiNotes.Length > 0 && _noteIndex < MidiNotes.Length)
		{
			...
			while (_noteIndex < MidiNotes.Length && MidiNotes[_noteIndex].StartTime < _timer)
			{
				...
				{
					if (ShowMIDIChannelColours && MIDIChannelColours != null && MIDIChannelColours.Length > 0)
					{
						// Ensure channel index is within bounds
						int channelIndex = MidiNotes[_noteIndex].Channel % MIDIChannelColours.Length;
						PianoKeyDetector.PianoNotes[MidiNotes[_noteIndex].Note].Play(MIDIChannelColours[channelIndex],
