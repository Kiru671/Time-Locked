...
public class PianoKey : MonoBehaviour
{
	...
	// SequenceTracker için debounce
	private float _lastSequenceNotifyTime = -1f;
	private const float SEQUENCE_DEBOUNCE_TIME = 0.1f; // 100ms debounce
	...
	void Update()
	{
		...
		if (PianoKeyController.KeyMode == KeyMode.Physical)
		{
			if (transform.eulerAngles.x > 350 && transform.eulerAngles.x < 359.5f && !_played)
			{
				...
				// Fiziksel tuş basıldığında SequenceTracker'ı bilgilendir
				NotifySequenceTracker();
	...
	public void Play(float velocity = 10, float length = 1, float speed = 1)
	{
		...
		// SequenceTracker'a nota çalındığını bildir
		NotifySequenceTracker();
	...
	private void NotifySequenceTracker()
	{
		// Debounce kontrolü - aynı notayı çok kısa sürede tekrar çalmayı engelle
		if (Time.time - _lastSequenceNotifyTime < SEQUENCE_DEBOUNCE_TIME)
		{
			return;
		}

		// SequenceTracker varsa ve aktifse nota ismini gönder
		if (SequenceTracker.Instance != null && PianoKeyController != null)
		{
			// Bu piano key'in nota ismini bul
			string noteName = GetNoteName();
			if (!string.IsNullOrEmpty(noteName))
			{
				_lastSequenceNotifyTime = Time.time;
				SequenceTracker.Instance.OnNotePressed(noteName);
			}
		}
	}
	...
	{
		// PianoKeyController'daki dictionary'den bu key'in nota ismini bul
		if (PianoKeyController != null && PianoKeyController.PianoNotes != null)
		{
